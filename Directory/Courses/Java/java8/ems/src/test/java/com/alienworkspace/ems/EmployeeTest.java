...
public class EmployeeTest {
    ...
    public static void main(String arg[]) {
        ...
        List<Employee> allInEdc = getAllEmployeeFilterBy(new EmployeeUnitFilter(Unit.EDC));
        System.out.println(allInEdc);

        // 2. Get me list of all employees in Financial Services.
        System.out.println("------All in FS------");

        List<Employee> allInFS = getAllEmployeeFilterBy(new EmployeeUnitFilter(Unit.FS));
        System.out.println(allInFS);

        // 3. Get all employees with java skills.
        System.out.println("--------All java developers-------");

        List<Employee> allJavaDevelopers = getAllEmployeeFilterBy(new EmployeeSkillFilter(Skill.JAVA));
        ...
        List<Employee> allDevelopersWithYearsOfExperience = getAllEmployeeFilterBy(new EmployeeYearsOfExperienceFilter(3));
        System.out.println(allDevelopersWithYearsOfExperience);
    }

    private static List<Employee> getAllEmployeeFilterBy(EmployeeFilter employeeFilter) {
...
//        return allEmployees;
        ...
        return employees.stream().filter(employeeFilter::filter).collect(Collectors.toList());
    }
//
//    private static List<Employee> getAllSkillDevelopers(EmployeeFilter employeeFilter) {
//        List<Employee> employeesWithJava = new ArrayList<>();
//        for (Employee employee : employees) {
//            if (employeeFilter.filter(employee)) {
//                employeesWithJava.add(employee);
//            }
//        }
//        return employeesWithJava;
//    }
//
//    private static List<Employee> getAllEmployeesInUnit(EmployeeFilter employeeFilter) {
...
//    private static List<Employee> getAllEmployeesInEDC() {
